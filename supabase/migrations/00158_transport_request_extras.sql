-- 00158_transport_request_extras.sql
--
...
--
-- НОМЕР. Файл собирался как 00156, но этот номер уже занял
-- 00156_activity_log_offsets из параллельной работы по взаимозачётам,
-- приехавшей в main тем же днём. Два файла с одним номером — это
-- неоднозначный порядок применения, поэтому файл переименован в 00158.
-- Содержимое не менялось; 00157 (коды груза) от него не зависит, а он
-- не зависит от 00157 — порядок между ними безразличен.
