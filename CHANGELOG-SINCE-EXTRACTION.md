...
- **#8 — единый формат дат ДД.ММ.ГГ (2-значный год).** Новая утилита `formatDMY(iso)` в `src/lib/format.ts` (`"2026-06-17" → "17.06.26"`, работает и с ISO-таймстампом, без `new Date` для голой даты — нет UTC-сдвига). Переведены все экранные даты: `registry/page.tsx` (`fmtDate`), `quotations/page.tsx` (`formatDateDMY`), `dt-kt/page.tsx`, `applications/page.tsx`, `components/deals/deal-payments.tsx`, `deal-lines-editor.tsx` (2 места), `deal-shipments.tsx` (`fmtDate`), `deal-trigger-prices.tsx` (`formatDate`), `components/shared/audit-history.tsx` (ветка чистой даты). НЕ тронуты: ленты активности с датой+временем (другой стиль), `fmtTs`, месяц-названия, Excel-экспортёры.
