...
- **Сомнение:** под-строки платежей в этом файле берут курс по ВАЛЮТЕ СДЕЛКИ (`d.supplier_currency`/`d.buyer_currency`), а не по валюте конкретного платежа — локальный `fetchPaymentsByDeals` в `passport-detail-excel.ts` не выбирает колонку `currency` у `deal_payments` (только `deal_id, side, amount, payment_date, payment_type`), в отличие от `fetchDealEvents` (Task 3), который её тянет и учитывает в агрегатах сделки через `convertDeal`. Итоговая строка сделки (Оплата) может НЕ совпасть с суммой под-строк-оплат, если хотя бы один платёж по сделке хранит валюту, отличную от валюты сделки. Код соответствует брифу дословно — не менял, т.к. задача не просила трогать select платежей; если это встречается на реальных данных, нужен отдельный фикс (добавить `currency` в select + PaymentLite).
