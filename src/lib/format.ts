 ...
 *
 * Правила (утверждено клиентом 2026-07-07, деньги пересмотрены 2026-09-08):
 * • Деньги (сумма, оплата, баланс, долг, тариф, скидка, котировка,
 *   FX-курс) — 3 знака после запятой на операционных экранах: сделки,
 *   реестр, ДТ-КТ и выгрузки в Excel из них (клиент 2026-09-08: «во
 *   всех числах, связанных с ценой (деньгами), в сделках и в реестре
 *   после запятой должны быть 3 знака»). `formatMoney` ниже остался
 *   на 2 знаках и обслуживает только счета-фактуры, где сумма
 *   бухгалтерская.
 * • Цена за тонну — 3 знака всегда (клиент 2026-09-04); с 2026-09-08
 *   совпадает с общим правилом для денег.
 * • Тонны / объёмы — 3 знака после запятой всегда. Объём НЕ показываем
 *   точнее трёх знаков (решение клиента 2026-09-08), хотя в базе он
 *   хранится с шестью: сумма считается по полному значению, поэтому
 *   «цена × объём», пересчитанные с экрана, могут разойтись с суммой
 *   на копейки.
...
/**
 * Деньги в бухгалтерских документах (счета-фактуры) — 2 знака.
 * Операционные экраны (сделки, реестр, ДТ-КТ) считают деньги по 3
 * знака своими локальными хелперами — см. шапку файла.
 */
