 ...
 *
 * Правила (утверждено клиентом 2026-07-07, деньги пересмотрены 2026-09-22):
 * • СУММЫ — 2 знака после запятой: сумма отгрузки и прихода, сумма по
 *   тоннажу, сумма ЖД, сумма грузоотправления, сумма договора, оплата,
 *   взаимозачёт, возврат, баланс, долг, сальдо, штрафы, сверхнормативы,
 *   ОГЭМ. Везде: сделки, реестр, ДТ-КТ, отчёты и выгрузки в Excel.
 *   `formatMoney` ниже — канон для сумм (и для счетов-фактур, у них
 *   всегда было 2 знака).
 * • СТАВКИ ЗА ЕДИНИЦУ — 3 знака: цена $/т, котировка, скидка, тариф
 *   (логисты / ЖД поставщика / грузоотправления / факт), FX-курс,
 *   коэффициент барелизации. Клиент перемножает «цена × объём» тем
 *   числом, которое видит на экране, поэтому ставку нельзя печатать
 *   короче, чем она участвует в расчёте (00166: цена отгрузки в базе
 *   тоже округлена до 3 знаков).
 * • Тонны / объёмы — 3 знака после запятой всегда. Объём НЕ показываем
 *   точнее трёх знаков (клиент 2026-09-08), хотя в базе он хранится с
 *   шестью.
...
/**
 * Суммы — 2 знака (клиент 2026-09-22). Счета-фактуры форматируются
 * этим же хелпером: у них 2 знака были всегда. Ставки за единицу
 * (цена, котировка, тариф, курс) и объёмы — 3 знака, см. шапку файла.
 */
