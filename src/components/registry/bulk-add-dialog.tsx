...
// Auto-tick «Продублировать отгрузку» when:
//   (a) BOTH chain positions 1 AND 2 are «ОсОО» / «Singularity»
//       (operator request 2026-06-24), OR
//   (b) BOTH chain positions 1 AND 2 are in the 12-name whitelist
//       below (operator request 2026-06-25).
// The dup checkbox stays user-toggleable — this only changes the
// default. Matching is case-insensitive and ignores whitespace +
// hyphens (so «Дот-Трейдинг» / «Дот Трейдинг» both match).
...
const _AUTO_DUP_WHITELIST_NORM = [
  "caodl",
  "fuelsupplycompany",
  "geowax",
  "kerneltradegmbh",
...
function _isWhitelistGroup(g: { name?: string | null; full_name?: string | null } | null | undefined): boolean {
  if (!g) return false;
  const h = _normalizeForWhitelist(`${g.full_name ?? ""}${g.name ?? ""}`);
  return _AUTO_DUP_WHITELIST_NORM.some((w) => h.includes(w));
}
function _shouldAutoDupChain(
...
export function BulkAddDialog({
  ...
  // ВТД — номер документа, общий на всю партию вагонов (00169,
  // клиент 2026-09-24: «нету ВТД» в массовом добавлении).
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setDupShipment(_shouldAutoDupChain(pos1, pos2));
    });
  }, [open, context?.dealId]);
          ...
          <div className="rounded border border-amber-200 bg-amber-50/30 p-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2">
              ...
              <div>
                ...
                <Input value={vtd} onChange={(e) => setVtd(e.target.value)} className="h-8 text-[12px] font-mono" placeholder="Необязательно" />
              </div>
              <div>
              ...
              {/* Выбор варианта цены («домика»). Показываем всегда, когда
                  ...
                  варианте поле тоже видно — из него понятно, на какую цену
                  сядет отгрузка, а второй вариант заводится в карточке
                  сделки (клиент 2026-09-24: «нету выбора приложения»). */}
              {supLines.length > 0 && (
                <div>
