...
const columns: ColumnDef<Consignee, unknown>[] = [
  {
    accessorKey: "name",
    header: "Наименование",
...
type FormProps = {
  item: Consignee | null;
  onSave: (values: Partial<Consignee>) => Promise<void>;
  onClose: () => void;
};

function ConsigneeForm({ item, onSave, onClose }: FormProps) {
  ...
  function set(key: keyof Consignee, value: string | boolean) {
    ...
    <form onSubmit={handleSubmit} className="space-y-4 pt-2">
      <div className="space-y-1.5">
        ...
        <Input
          ...
          placeholder="Название грузополучателя"
        />
      </div>
  ...
  return (
    <div className="space-y-6">
      <CrudTable<Consignee>
        ...
        searchPlaceholder="Поиск грузополучателя..."
        onSave={save}
        onDelete={remove}
        renderForm={({ item, onSave, onClose }) => (
          <ConsigneeForm item={item} onSave={onSave} onClose={onClose} />
        )}
      />
    </div>
  );
