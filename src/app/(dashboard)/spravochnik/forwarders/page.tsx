...
const columns: ColumnDef<Forwarder, unknown>[] = [
  {
    accessorKey: "name",
    header: "Наименование",
...
type FormProps = {
  item: Forwarder | null;
  onSave: (values: Partial<Forwarder>) => Promise<void>;
  onClose: () => void;
};
...
function ForwarderForm({ item, onSave, onClose }: FormProps) {
  ...
  function set(key: keyof Forwarder, value: string | boolean) {
    ...
    <form onSubmit={handleSubmit} className="space-y-4 pt-2">
      <div className="space-y-1.5">
        ...
        <Input
          ...
          placeholder="Название экспедитора"
        />
      </div>
    ...
    <div className="space-y-6">
      <CrudTable<Forwarder>
        ...
        renderForm={({ item, onSave, onClose }) => (
          <ForwarderForm item={item} onSave={onSave} onClose={onClose} />
        )}
      />
    </div>
  );
